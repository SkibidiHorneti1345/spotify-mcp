...
A robust, production-grade Model Context Protocol (MCP) server that connects Spotify playback, search, and library management directly to LLM Agents.
...
- **Playback Control**: Play tracks/albums/playlists, pause, skip, adjust volume, toggle shuffle/repeat, transfer playback between devices.
- **Search & Discovery**: Search tracks, artists, albums, playlists, and get tailored music recommendations based on genres/artists.
...
2. Once authorized, the Spotify callback redirects to `http://localhost:8080/callback?code=...` which is processed, and tokens are written to `.spotify_token_cache` in the project root.
...
3. Open the link, log in, authorize, and copy the final URL you redirect to (e.g., `http://localhost:8080/callback?code=...`).
